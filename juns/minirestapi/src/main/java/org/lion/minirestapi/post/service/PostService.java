...
public class PostService {
    ...
    public Page<PostDetailResponseDTO> findAllAdmin(User loginUser, @NonNull Pageable pageable) {
        boolean isAdmin = loginUser.getRoles().stream().map(Role::getName).anyMatch(role -> role.equals("ADMIN"));
    ...
    public PostDetailResponseDTO findPostById(@NonNull Long postId) {
        Post post = postRepository.findByIdAndActivateTrue(postId).orElseThrow(() -> new PostNotFoundException(ErrorString.POST_NOT_FOUND.getMessage()));
        return PostDetailResponseDTO.fromEntity(post);
    }

    @Transactional
    public void deactivatePostById(@NonNull Long postId, User loginUser) {
        Post post = postRepository.findByIdAndActivateTrue(postId).orElseThrow(() -> new PostNotFoundException(ErrorString.POST_NOT_FOUND.getMessage()));
    ...
    @Transactional
    public void deletePostById(@NonNull Long postId, User loginUser) {
        Post post = postRepository.findById(postId).orElseThrow(() -> new PostNotFoundException(ErrorString.POST_NOT_FOUND.getMessage()));
        boolean isOwner = loginUser.getId().equals(post.getUser().getId());
        boolean isAdmin = loginUser.getRoles().stream().map(Role::getName).anyMatch(role -> role.equals("ADMIN"));
        if(!isOwner && !isAdmin) {
            throw new AccessDeniedException(ErrorString.ACCESS_DENIED.getMessage());
        }
    ...
    @Transactional
    public PostDetailResponseDTO updatePostById(@NonNull Long postId, @NonNull PostUpdateDTO postUpdateDTO, User loginUser) {
        Post post = postRepository.findByIdAndActivateTrue(postId).orElseThrow(() -> new PostNotFoundException(ErrorString.POST_NOT_FOUND.getMessage()));
        if(!post.getUser().getId().equals(loginUser.getId())) {
            throw new AccessDeniedException(ErrorString.ACCESS_DENIED.getMessage());
        }
