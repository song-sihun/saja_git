    ...
    fetch('/api/products')
        ...
        .then(data => {
            ...
            data.forEach(product => {
                ...
                item.innerHTML = `Name: ${product.productName}, Price: ${product.price}
                                  <button onclick="editProduct(${product.id}, '${product.productName}', ${product.price})">Edit</button>
...
function editProduct(id, oldName, oldPrice) {
    ...
    if (newName !== null && newPrice !== null) {
        fetch(`/api/products/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ productName: newName, price: parseFloat(newPrice) })
        })
            .then(response => response.json())
